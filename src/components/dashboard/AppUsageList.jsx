...
export function AppUsageList({ className }) {
  ...
  useEffect(() => {
    ...
    const handleAppUsageUpdate = (appUsage) => {
      console.log("Raw app usage data received:", appUsage);
      
      // Convert to formatted app usage items with proper time formatting
      const formattedAppUsage = appUsage.map(app => ({
        name: app.name,
        time: app.time, // Keep the raw time in milliseconds
        type: app.type,
        lastActiveTime: app.lastActiveTime
      }));
      
      console.log("Formatted app usage data:", formattedAppUsage);
  ...
  // Fixed time formatting function to handle milliseconds properly
  const formatTime = (ms) => {
    console.log("Formatting time for ms:", ms);
    ...
    console.log("Time breakdown - hours:", hours, "minutes:", minutes, "seconds:", seconds);
  ...
  const isAppWhitelisted = (appName) => {
